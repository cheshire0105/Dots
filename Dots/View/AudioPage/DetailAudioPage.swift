...
class DetailAudioPage: UIViewController {
    ...
    private func setupButtons() {
        ...
        leftButton.backgroundColor = UIColor(red: 0.412, green: 0.412, blue: 0.412, alpha: 1)
           ...
           // 가운데 버튼 설정
           if let centerImage = UIImage(named: "Group 150") { // 'yourImageName'은 에셋 이미지의 이름입니다.
               centerButton.setImage(centerImage, for: .normal)
               centerButton.backgroundColor = .white
           }
           configureButtonAppearance(button: centerButton)
...
//           button.backgroundColor = .white
    ...
    private func setupCloseButton() {
        ...
        if let image = UIImage(named: "Group 149") { // 'closeIcon'은 닫기 버튼으로 사용할 이미지의 이름입니다.
    ...
    private func setupRoundedRectangleView() {
        ...
        roundedRectangleView.snp.makeConstraints { make in
            make.top.equalTo(closeButton.snp.top).offset(40) // 'closeButton' 아래에 위치
            make.left.equalTo(view.safeAreaLayoutGuide.snp.left).offset(20)
